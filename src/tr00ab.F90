...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
  ...
  real(wp), intent(inout)  :: ldiota(0:1,-1:2)

  integer              :: innout, ll, ii, jj, kk, jb, kb, mj, nj, ideriv, jderiv, id, MM, ielement, nelements, Lcurvature, idof, icon, mi, ni, imupf

  real(wp)                 :: lcpu, mfactor, lss, Dteta, Dzeta, rfac, tol, rnorm, omega, diotaerror!, sparsedenseerror

  real(wp)                 :: lAte(0:mn,-1:2), lAze(0:mn,-1:2), lAto(0:mn,-1:2), lAzo(0:mn,-1:2)
...
! required for Fourier routines;
  ...
  real(wp)  , allocatable  :: dmatrix(:,:,:), omatrix(:,:), FAA(:,:)
  real(wp)                 :: drhs(1:NN,-1:2), dlambda(1:NN,-1:2)
  real(wp)                 :: Rdgesvx(1:NN), Cdgesvx(1:NN), work4(1:4*NN), rcond, ferr(1), berr(1), ferr2(1:2), berr2(1:2)
  character            :: equed
...
!required for SVD routines;
  ...
  real(wp)                 ::                      Bsupt(1:Nt*Nz,-1:2), Bsupz(1:Nt*Nz,-1:2), tdot(1:Nt*Nz)
  real(wp)                 :: Bsubs(1:Nt*Nz,-1:2), Bsubt(1:Nt*Nz,-1:2), Bsubz(1:Nt*Nz,-1:2)

  real(wp)                 :: dotteta, dotzeta

  real(wp)   , allocatable :: rmatrix(:,:,:), rrhs(:,:), rlambda(:,:), wks1(:), wks2(:), AA(:,:)

  integer              :: inz(-1:2), lnz
  integer, allocatable :: irow(:,:), jcol(:,:), istr(:), iwork(:)
  real(wp)   , allocatable :: smatrix(:,:), srhs(:,:), slambda(:,:), swork(:)
...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
   ...
   if( mns.le.0 ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; mns.le.0 ; no degrees of freedom in angle transformation ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : mns.le.0 : no degrees of freedom in angle transformation  ;"
    endif
 ! this is only for Fourier; 20 Apr 13;

   if( lvol.lt.1 .or. lvol.gt.Mvol ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; lvol.lt.1 .or. lvol.gt.Mvol ; illegal lvol ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : lvol.lt.1 .or. lvol.gt.Mvol : illegal lvol  ;"
    endif


   if( iflag.lt.-1 .or. iflag.gt.2 ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; iflag.lt.-1 .or. iflag.gt.2 ; illegal iflag ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : iflag.lt.-1 .or. iflag.gt.2 : illegal iflag  ;"
    endif
...
! construct real-space, real-space transformation matrix; 20 Apr 13;
   ...
   if( NOTstellsym ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; NOTstellsym ; under construction ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : NOTstellsym : under construction  ;"
    endif


   if( Ntor.ne.0   ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; Ntor.ne.0   ; under construction ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : Ntor.ne.0   : under construction  ;"
    endif
   ...
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; iorder not supported ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : iorder not supported  ;"
    endif
...
!      if( jj.ge.hNt .and. kk.eq.hNz ) cycle
   ...
   if( ii.ne.Ndof ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; ii.ne.Ndof ; counting error ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : ii.ne.Ndof : counting error  ;"
    endif
...
! dense arrays; 24 Apr 13; ! these will eventually be redundant; 24 Apr 13;
   ...
   allocate( rmatrix(1:Ndof,1:Ndof,-1:2), stat=astat )
   rmatrix(1:Ndof,1:Ndof,-1:2) = zero
 ! real-space angle transformation matrix; dense; 23 Apr 13;

   allocate( rrhs   (1:Ndof,       -1:2), stat=astat )
   rrhs   (1:Ndof,       -1:2) = zero


   allocate( rlambda(1:Ndof,       -1:2), stat=astat )
   rlambda(1:Ndof,       -1:2) = zero


   allocate( wks1   (1:Ndof            ), stat=astat )
   wks1   (1:Ndof            ) = zero


   allocate( wks2   (1:Ndof            ), stat=astat )
   wks2   (1:Ndof            ) = zero


   allocate( AA     (1:Ndof,1:Ndof     ), stat=astat )
   AA     (1:Ndof,1:Ndof     ) = zero
...
! sparse arrays; ! all of these can be simply defined (1:Ntz) etc. . . . ; 24 Apr 13;
   ...
   allocate( srhs   (1:  Ndof  ,-1:2), stat=astat )
   srhs   (1:  Ndof  ,-1:2) = zero


   allocate( istr   (1:  Ndof+1     ), stat=astat )
   istr   (1:  Ndof+1     ) = 0
 ! for re-ordering; 24 Apr 13;

   allocate( iwork  (1:2*Ndof+1     ), stat=astat )
   iwork  (1:2*Ndof+1     ) = 0
 ! for re-ordering & iterative solver; 24 Apr 13;

   allocate( slambda(1:  Ndof  ,-1:2), stat=astat )
   slambda(1:  Ndof  ,-1:2) = zero

     select case( iorder )
     case( 2 )

   allocate( smatrix(1:Ndof*5,-1:2), stat=astat )
   smatrix(1:Ndof*5,-1:2) = zero
 ! real-space angle transformation; sparse; 24 Apr 13;

   allocate( irow   (1:Ndof*5,-1:2), stat=astat )
   irow   (1:Ndof*5,-1:2) = 0


   allocate( jcol   (1:Ndof*5,-1:2), stat=astat )
   jcol   (1:Ndof*5,-1:2) = 0

     case default

   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; need to estimate length of smatrix irow and jcol ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : need to estimate length of smatrix irow and jcol  ;"
    endif
   ...
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; selected value of iorder not supported ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : selected value of iorder not supported  ;"
    endif
...
! REAL MATRICES HAVE BEEN CONSTRUCTED; 24 Apr 13;
     ...
     lcpu = MPI_WTIME() ! record time taken in F11DEF; 20 Apr 13;
     ...
     case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal ifail returned by F11ZAF ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal ifail returned by F11ZAF  ;"
    endif
...
! for safety, let's assume that the following NAG routines will destroy the provided matrices, so better keep a backup; 24 Apr 13;
    ...
    if( Lsparse.ge.2 ) then
     ...
     case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal iprecon ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal iprecon  ;"
    endif
     ...
     case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal imethod ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal imethod  ;"
    endif

     end select


   allocate( swork(1:lrwork), stat=astat )
   swork(1:lrwork) = zero
    ...
    !if( Lscalarpotential .and. ideriv.eq.2 ) cycle ! do not need derivatives of transform wrt enclosed poloidal current = "linking current" provided by coils;
     ...
     case( 1:2 ) ! matrix perturbation: A . \delta x = \delta b - \delta A . x ; 21 Apr 13;
      ...
      if( Lsparse.ge.2 ) then
       ...
       case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal ifail returned from F11XAF ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal ifail returned from F11XAF  ;"
    endif
...
1015   format("tr00ab : ", 10x ," : myid=",i3," ; lvol=",i3," ; innout="i2" ; ideriv="i2" ; if11xaf="i2" ;      "10x" ; "a13)
   ...
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; invalid ideriv ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : invalid ideriv  ;"
    endif
     ...
     if( Lsparse.ge.2 ) then ! use sparse solver; 24 Apr 13;
      ...
      case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal ifail returned by f11def ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal ifail returned by f11def  ;"
    endif
...
" ; its="i6" rnorm="es13.5" ;")
     ...
     if( Lsparse.eq.1 ) then ! use dense-solver; 24 Apr 13;
      ...
      case default ;
   if( .true. ) then
     write(6,'("tr00ab :      fatal : myid=",i3," ; .true. ; illegal ifail returned by f04atf ;")') myid
     call MPI_ABORT( MPI_COMM_SPEC, 1, ierr )
     stop "tr00ab : .true. : illegal ifail returned by f04atf  ;"
    endif
...
!$OMP ATOMIC UPDATE
 ...
 ! THIS CAN BE DELETED EVENTUALLY; 02 Sep 14;
!$OMP ATOMIC UPDATE
       dmatrix(ii      ,jj      ,ideriv) = dmatrix(ii      ,jj      ,ideriv) + ( - mj * lAze(kk,ideriv) + nj * lAte(kk,ideriv) ) * half
       if( NOTstellsym) then
...
!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
   ...
   deallocate(rmatrix,stat=astat)


   deallocate(rrhs,stat=astat)


   deallocate(rlambda,stat=astat)


   deallocate(wks1,stat=astat)


   deallocate(wks2,stat=astat)


   deallocate(AA,stat=astat)

   endif

   if( Lsparse.ge.2 ) then

   deallocate(smatrix,stat=astat)


   deallocate(srhs,stat=astat)
